...
10. 在 MacBook 执行 `codexctl task review ISSUE_URL`，明确批准当前 PR/head/fingerprint 后执行 `codexctl task merge ISSUE_URL --expected-head SHA --expected-fingerprint FINGERPRINT`；确认 Worker 之后才关闭 Issue 并标记 `codex:completed`。
