...
6. 创建一个低风险测试 Issue，手工触发 Slot 1 的 Prompt，完成一次领取、checkpoint 和 direct-main 交付。
7. 在 Mac mini Codex App 中创建三个独立 Scheduled 任务，频率均为 10 分钟并错开约一分钟。
8. 先启用 Slot 1；确认空运行、任务运行和归档行为后，再启用 Slot 2、Slot 3。
9. 发布两个路径不重叠的测试 Issue，验证形成两个可见并行任务；再发布一个路径重叠任务，确认其保持 ready。
10. 保持 Mac mini 开机、系统不睡眠且 Codex App 持续运行。
